...
class App extends React.Component{
  ...
  render(){
    return (<div className="app">
      ...
      <div>
        <input type="text" placeholder="Search from placeholder..." value={this.state.location} 
        onChange={((e) => this.setState({location: e.target.value}))} />
      </div>
      <button onClick={this.fetchWeather}>Get weather</button>
    {this.state.isLoading && <p className="loadder">Loading...</p>}
...
class Weather extends React.Component{
  render(){
    ...
    return (<div>
      ...
      {dates.map((date, i) => <Day date={date} max={max.at(i)} min={min.at(i)} code={codes.at(i)} key={i}
      isToday={i === 0}
